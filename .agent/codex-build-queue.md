...
- **Drift-test implication:** any vitest asserting `status === 401` for these RPCs (e.g., `tests/unit/gateway-rpc-grants-drift.test.ts` referenced in line 424) would falsely fail despite the lockdown holding. Sibling tests should accept either `401` OR `404+PGRST202` as the locked-state assertion. Codex follow-up: audit existing drift tests for status-code assumptions; widen to `[401, 404]` with body-content discriminator.
