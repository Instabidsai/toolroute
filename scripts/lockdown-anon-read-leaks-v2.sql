...
--
-- 2. Client-readable owner-scoped tables (src/app/dashboard/page.tsx is a
--    "use client" component that reads these via the public anon client with
--    the user's JWT, then filters by session.user.id). Blanket REVOKE would
--    silently empty the dashboard (Hard Rule #54). Instead: enable RLS and
--    add an owner-scoped SELECT policy.
...
FROM pg_policies
WHERE schemaname = 'public'
  AND tablename IN ('gateway_users', 'gateway_usage_log')
ORDER BY tablename, policyname;
...
SELECT
  ...
  has_table_privilege('anon', 'public.' || t, 'SELECT') AS anon_select,
  has_table_privilege('authenticated', 'public.' || t, 'SELECT') AS auth_select
FROM unnest(ARRAY['gateway_users', 'gateway_usage_log']) AS t;
