...
// Lane 4.215 — drift guard: tool_providers.created_at
// SELECT-projection allow-list.
//
// `tool_providers.created_at` is the master-key-provisioning
// timestamp on the gateway-owned provider registry. Pairing
// `created_at` with `tool_slug` + `cost_per_call` (4.186) +
// `markup_percent` (4.188) + `health_status` (4.194) reconstructs
// the full provisioning history of every master credential —
// when each provider was wired up, what economics, what reliability.
// Admin-only inventory signal.
//
...
// (Lanes 4.210/4.211/4.212/4.213/4.214).
//
...
// Files allowed to SELECT `created_at` from `tool_providers`.
// Each entry has a real reason — see lane comment above.
const PROJECTION_ALLOWLIST = new Set<string>([
  "src/app/api/admin/providers/route.ts",
]);

describe("Lane 4.215 — tool_providers.created_at SELECT-projection allow-list", () => {
  const files = walk(SRC_ROOT);

  it("only allow-listed files SELECT created_at from tool_providers", () => {
    const re =
      /\.from\(\s*["']tool_providers["']\s*\)[\s\S]{0,500}?\.select\(\s*["'`][^"'`]*\bcreated_at\b[^"'`]*["'`]/;
  ...
  it("only allow-listed files declare created_at in a tool_providers .returns<>() generic", () => {
    const re =
      /\.from\(\s*["']tool_providers["']\s*\)[\s\S]{0,500}?\.returns<[\s\S]*?\bcreated_at\b/;
    const violators: string[] = [];
    for (const file of files) {
      const src = stripComments(readFileSync(file, "utf-8"));
      if (re.test(src)) {
  ...
  it("no raw SQL SELECT created_at FROM tool_providers in src/", () => {
    const re =
      /SELECT\s+[\s\S]*?\bcreated_at\b[\s\S]*?\bFROM\s+tool_providers\b/i;
    const violators: string[] = [];
    for (const file of files) {
      const src = stripComments(readFileSync(file, "utf-8"));
