...
// Lane 4.227 — drift guard: DEEPGRAM_API_KEY env-var direct-read
// allow-list.
//
// `DEEPGRAM_API_KEY` is the master credential for ToolRoute's
// Deepgram speech-to-text adapter. ToolRoute charges per-call
...
//
//   1. BYOK preference — `byokKey || process.env.DEEPGRAM_API_KEY`
...
//
//   3. Rate-limit accounting — Deepgram caps are per-key. A second
...
//
//   - src/lib/adapters/deepgram-adapter.ts — the canonical
//     deepgram adapter. Implements the BYOK fallback at line 8
//     (`byokKey || process.env.DEEPGRAM_API_KEY || null`). The
//     error-message string-literal reference on line 28 is not
//     an env read.
//
// Out-of-scope (not a process.env read):
//
//   - src/lib/adapter-availability.ts:19 — `deepgram: ["DEEPGRAM_API_KEY"]`
...
// 4.225 (SUPABASE_MGMT_TOKEN), 4.226 (ELEVENLABS_API_KEY).
//
...
// Files allowed to read `DEEPGRAM_API_KEY` from process.env.
// Each entry has a real reason — see lane comment above.
const ENV_READ_ALLOWLIST = new Set<string>([
  "src/lib/adapters/deepgram-adapter.ts",
]);

describe("Lane 4.227 — DEEPGRAM_API_KEY env-var direct-read allow-list", () => {
  const files = walk(SRC_ROOT);

  it("only allow-listed files read process.env.DEEPGRAM_API_KEY", () => {
    // Match `process.env.DEEPGRAM_API_KEY` (dot access) and
    // `process.env["DEEPGRAM_API_KEY"]` (bracket access).
    const re =
      /process\.env\s*(?:\.\s*DEEPGRAM_API_KEY\b|\[\s*["']DEEPGRAM_API_KEY["']\s*\])/;
  ...
  it("no destructured `const { DEEPGRAM_API_KEY } = process.env` outside allow-list", () => {
    // Destructuring assignment leaks the same value but evades dot/bracket
    // access regex. Match `{ DEEPGRAM_API_KEY` ... `} = process.env`.
    const re =
      /\{\s*[^}]*\bDEEPGRAM_API_KEY\b[^}]*\}\s*=\s*process\.env/;
    const violators: string[] = [];
    for (const file of files) {
      const src = stripComments(readFileSync(file, "utf-8"));
      if (re.test(src)) {
