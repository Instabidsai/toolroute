...
// Lane 4.231 — drift guard: TAVILY_API_KEY env-var direct-read
// allow-list.
//
// `TAVILY_API_KEY` is the master credential for ToolRoute's
// Tavily adapter (slug `tavily`). Tavily is a paid-only research-
// search API (no free tier) and is one of two Search-tier
// adapters on the catalog (alongside Brave Search). ToolRoute
// charges per-call against this master pool when end-users
// haven't supplied a BYOK key (gating-aware adapter wrapper,
// Lane 4.103). Direct env-var reads from any new file silently
// bypass:
//
//   1. BYOK preference — `byokKey || process.env.TAVILY_API_KEY`
...
//      Bypass readers don't get attributed → revenue leak. Tavily
//      has no free tier, so every bypass call burns master-pool
//      cash with zero offsetting end-user charge.
//
//   3. Rate-limit accounting — Tavily caps are per-key. A second
//      module dialing the same master key risks consuming quota
//      that the canonical adapter believes is available.
//
//   4. CRED-LEAK CLASS PRECEDENT — Lane 4.76 already had to
//      remediate a Tavily credential body-leak (key was being
//      echoed in error responses). Tighter env-read perimeter
//      shrinks the surface where that class of bug can recur.
//
// Today's env-var read surface is exactly ONE file:
//
//   - src/lib/adapters/tavily-adapter.ts — the canonical Tavily
//     adapter (slug `tavily`). Implements the BYOK fallback at
//     line 8 (`byokKey || process.env.TAVILY_API_KEY || null`).
//     The error-message string-literal reference on line 28 is
//     not an env read.
//
// Out-of-scope (not a process.env read):
//
//   - src/lib/adapter-availability.ts:52 —
//     `tavily: ["TAVILY_API_KEY"]` is a string literal in the
...
// Files allowed to read `TAVILY_API_KEY` from process.env.
// Each entry has a real reason — see lane comment above.
const ENV_READ_ALLOWLIST = new Set<string>([
  "src/lib/adapters/tavily-adapter.ts",
]);

describe("Lane 4.231 — TAVILY_API_KEY env-var direct-read allow-list", () => {
  const files = walk(SRC_ROOT);

  it("only allow-listed files read process.env.TAVILY_API_KEY", () => {
    // Match `process.env.TAVILY_API_KEY` (dot access) and
    // `process.env["TAVILY_API_KEY"]` (bracket access).
    const re =
      /process\.env\s*(?:\.\s*TAVILY_API_KEY\b|\[\s*["']TAVILY_API_KEY["']\s*\])/;
  ...
  it("no destructured `const { TAVILY_API_KEY } = process.env` outside allow-list", () => {
    // Destructuring assignment leaks the same value but evades dot/bracket
    // access regex. Match `{ TAVILY_API_KEY` ... `} = process.env`.
    const re =
      /\{\s*[^}]*\bTAVILY_API_KEY\b[^}]*\}\s*=\s*process\.env/;
    const violators: string[] = [];
    for (const file of files) {
      const src = stripComments(readFileSync(file, "utf-8"));
      if (re.test(src)) {
